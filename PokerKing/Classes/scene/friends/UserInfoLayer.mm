...
SEL_MenuHandler UserInfoLayer::onResolveCCBCCMenuItemSelector(CCObject * pTarget, CCString * pSelectorName) {
    ...
    CCB_SELECTORRESOLVER_CCMENUITEM_GLUE(this, "onWeiboCliecked", UserInfoLayer::onWeiboClicked);
    CCB_SELECTORRESOLVER_CCMENUITEM_GLUE(this, "onFriendClicked", UserInfoLayer::onFriendClicked);
...
void UserInfoLayer::onFriendClicked(cocos2d::CCObject *pSender)
{
    Loading::sharedLoading()->addLoadingToLayer(this);
    
    GameServerAction::sharedGameServerAction()->sendFriendInvitation(mUserInfo->getUserID(), this, callfuncND_selector(UserInfoLayer::sendInvitation_Done));
}

void UserInfoLayer::sendInvitation_Done(cocos2d::CCNode * pNode, void *data)
{
    std::string msg = "";
...
void UserInfoLayer::onWeiboClicked(cocos2d::CCObject *pSender)
{
    NSString * value = [[NSString alloc] initWithFormat:@"%s", mUserInfo->getSNS().c_str()];
    [[WeiboFactory sharedWeiboFactory] createFriend:value];
    
    AlertLayer * shareAlert = AlertLayer::create("关注微博", CCString::createWithFormat("你已成功关注 %s", mUserInfo->getUserName().c_str())->getCString(), false, NULL, NULL);
